...
# ComfyUI version compatibility check
def get_comfyui_version():
    """
    Detect ComfyUI version based on available API features.
    Returns: tuple (major, minor, patch) or (0, 0, 0) if detection fails
    """
    try:
        # Check for new ComfyUI 0.9.2+ features in prompt queue
        prompt_queue = server.PromptServer.instance.prompt_queue
        if hasattr(prompt_queue, 'currently_running'):
            # Try to detect version from queue structure
            if len(prompt_queue.currently_running) > 0:
                value = next(iter(prompt_queue.currently_running.values()))
                if len(value) >= 6:
                    return (0, 9, 2)  # ComfyUI 0.9.2+ with new queue format
                else:
                    return (0, 8, 0)  # Older ComfyUI
    except:
        pass
...
def is_comfyui_version_compatible(min_version=(0, 8, 0)):
    """
    Check if current ComfyUI version is compatible.
    ...
    """
    current = get_comfyui_version()
    return current >= min_version

# Log ComfyUI version on import
_comfyui_version = get_comfyui_version()
logger.info(f"ComfyUI version detected: {_comfyui_version[0]}.{_comfyui_version[1]}.{_comfyui_version[2]}")
if not is_comfyui_version_compatible():
    logger.warning("ComfyUI version may not be compatible with VideoHelperSuite")
